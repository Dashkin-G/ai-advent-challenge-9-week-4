...
SUMMARY_SYSTEM = (
    ...
    "и найденные строки: файл и номер строки, после слова «внутри» — функция или "
    "класс, где стоит строка, и сам текст. Напиши по-русски 3–6 коротких строк: что "
    "это такое, где объявлено, где и зачем используется. Имена файлов и функций пиши "
    "как есть. Опирайся только на эти строки и ничего не придумывай. Без вступлений "
    "и советов."
)
...
async def summarize(found: dict) -> dict:
    ...
    for m in matches:
        inside = ", внутри {0}".format(m["внутри"]) if m.get("внутри") else ""
